...
inline void init_game()
{
    ...
    // Texts
    ecs_entity_t scenario;
    ecs_create_entity(&scenario);
    ecs_add_component(scenario, transform_t, &((transform_t){ .position={ -7, -6, -0.2 } }));
    ecs_add_component(scenario, text_t, &((text_t){ 
                ...
                .color={ 1, 1, 1, 0 }, .size=0.5f, .camera=camera, .max_width=5}));
    ecs_add_component(scenario, animation_t, &((animation_t){ .callback=scenario_animation }));
    ...
    ecs_entity_t subtitle;
    ecs_create_entity(&subtitle);
    ecs_add_component(subtitle, transform_t, &((transform_t){ .position={ -4.5, -2, 0 } }));
    ecs_add_component(subtitle, text_t, &((text_t){ .color={ 1, 1, 1, 0 }, .text="Press ZQSD to move and ENTER when you're dead", .size=0.6, .camera=camera, .max_width=4 }));
    ecs_add_component(subtitle, animation_t, &((animation_t){ .callback=subtitle_animation }));
